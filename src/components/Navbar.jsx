...
import { navLinks } from '../constants'; // Ensure navLinks is updated
    ...
    <div className={style.navbar_container}>
      <nav className={`${style.navbar} ${scrolled ? 'fixed' : ''}`}>
        ...
        <ul className={style.navlink}>
          {navLinks.filter(link => link.name !== "Services" && link.name !== "Testimonial").map((link) => (
            <li
              key={link.id}
        ...
        >
          ...
          {navLinks.filter(link => link.name !== "Services" && link.name !== "Testimonial").map((link, index) => (
            <li
              key={link.id}
              className={active === link.name ? style.active : null}
            >
