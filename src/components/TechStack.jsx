...
const TechStack = () => {
  ...
  return (
    <>
      <motion.p variants={fadeIn('', '', 0.5, 1)} className={style.title}>
      ...
      <motion.div variants={textVariant()} className={style.btn_container}>
        {stacks.map((currentStack) => (
          ...
          >
            {currentStack.charAt(0).toUpperCase() + currentStack.slice(1)}
          </button>
        ))}
      </motion.div>
      ...
      <div className={style.tech_container}>
        {technologies
          .filter((technology) => stack === 'all' || technology.stack.includes(stack))
          .map((tech, index) => (
            <motion.abbr
              initial="hidden"
              whileInView="show"
